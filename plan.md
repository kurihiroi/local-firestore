...
## Phase 1: セキュリティルールの本家セマンティクス化（A-1, A-2, A-3）
...
## Phase 2: リミット強制とバリデーション（B-1〜B-4）
...
| 順序 | Phase | 理由 |
|---|---|---|
| 1 | Phase 1（ルール） | データ漏えい方向の差異を含み最重要。1-1 → 1-2 → 1-3 の順 |
| 2 | Phase 2（リミット） | 本番移行時の事故防止。独立性が高く並行着手可 |
| 3 | Phase 3（忠実度） | 保存形式の変更を伴うため、マイグレーション（3-4）を最後に一括提供 |
