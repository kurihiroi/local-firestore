...
Phase 1〜3 および Phase 4-1、Phase 5 は実装・配線・テスト済み。以下が未対応として残っている。

### 未対応項目

- **4-2. tsup splitting の有効化**: 全パッケージ `splitting: false` のまま。計画上も「検討」扱い。CJS ビルドでは splitting 不可のため、有効化する場合は ESM のみ対象とし動作確認が必要
- **6-1. マルチデータベース**: クライアントの `getFirestore(app, databaseId)` シグネチャと `_databaseId` の保持のみ実装済み。以下が未実装:
  - サーバー側のルーティング（`/databases/:dbId/docs/:path` 形式）
  - データベースごとの独立した SQLite ファイルの生成・管理
  - クライアントの `_databaseId` をリクエスト経路（transport）へ反映する処理
- **6-2. ベクトル検索**: `VectorValue` クラスと `vector()` ヘルパーのみ実装済み。以下が未実装:
  - `FindNearest` クエリ（クライアント API + サーバー側実装）
  - 距離計算（コサイン類似度 / ユークリッド距離）の SQLite ユーザー定義関数

### 軽微な残課題

- 型中心の項目（2-2 `SnapshotOptions`、2-4 `firestore`/`converter` getter、2-5 クエリ制約型）の専用テストが未整備（型互換のみで実害は小さい）
- `onSnapshot` の Observer 形式（1-7）で `complete` コールバックは型定義のみで呼び出されない（ローカルエミュレータには完了イベントが存在しないため実質仕様）
- Phase 5-1 のトリガー登録は Webhook（`POST /triggers`）と Node.js API（ライブラリ利用時）の2方式に対応済みだが、本家 Cloud Functions emulator のプロトコル互換はなし
