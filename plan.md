...
## 実装状況メモ（2026-07-04 時点）

Phase 1〜6 すべて実装・配線・テスト済み。

### 4-2. tsup splitting（対応済み）

- **server**: 複数エントリ（`index` / `cli`）を持つため、ESM ビルドのみ `splitting: true` を有効化（共有コードをチャンク化）。CJS は tsup/esbuild が code splitting 未対応のため無効のまま
- **shared / client**: 単一エントリのため splitting は効果がなく `splitting: false` のまま（検討の結果、見送り）
- 併せて、`package.json` の `exports.import` / `module` / `bin` が実際のビルド出力（`.js`）ではなく存在しない `.mjs` を指していた不整合を修正

### 6-1. マルチデータベース（実装済み）

- サーバー: `DatabaseManager` がデータベースIDごとに独立した SQLite ファイル（`store.db` → `store.<dbId>.db`、`:memory:` は独立インスタンス）と `ListenerManager` / `DocumentService` を遅延生成
- ルーティング: `/databases/:databaseId/*` を各データベースのサブアプリへディスパッチ（`(default)` はプライマリDBに解決）。無効なIDは 400
- WebSocket: サブスクライブメッセージの `databaseId` フィールドでリスナーをデータベースごとに分離
- クライアント: `getFirestore(settings, databaseId)` / `getFirestore(app, databaseId)` で `_databaseId` を保持し、非デフォルト時は transport の basePath（`/databases/:databaseId`）とサブスクライブメッセージへ反映

### 6-2. ベクトル検索（実装済み）

- クライアント: `findNearest(queryOrRef, { vectorField, queryVector, limit, distanceMeasure, distanceResultField?, distanceThreshold? })` が findNearest 制約付きの `Query` を返し、`getDocs` で実行可能。`VectorValue` は `{ __type: "vector", values: [...] }` 形式でシリアライズされる
- サーバー: SQLite ユーザー定義関数 `vector_distance` で EUCLIDEAN / COSINE / DOT_PRODUCT の距離を計算し、where フィルタと組み合わせて距離順に返す（DOT_PRODUCT は値が大きいほど近い扱い）。ベクトル未設定・次元不一致のドキュメントは対象外
