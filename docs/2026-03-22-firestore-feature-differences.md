...
| セキュリティ | セキュリティルール (完全なパーサー + 評価エンジン) | AST ベース。**適用対象は `/docs/*`（ドキュメント CRUD）のみ**。`/query` / `/aggregate` / `/batch` / `/transaction` / WebSocket リスナーはルール評価をバイパスする（plan.md 参照） |
| セキュリティ | クライアント SDK の認証トークン送信 (`FirestoreSettings.authTokenProvider`) | `Authorization: Bearer` ヘッダーで送信。サーバー側 `AUTH_PROVIDER=firebase` で Firebase Auth の ID トークンとして検証 |
...
3. **`getFirestore()` の第1引数** — 本家は `FirebaseApp` を受け取るが、local-firestore は `FirestoreSettings`（ホスト/ポート/authTokenProvider）を受け取る。`connectFirestoreEmulator()` は不要
...
8. **セキュリティルールの適用範囲** — ルール評価は `/docs/*`（ドキュメント CRUD）のみ。クエリ / 集計 / バッチ / トランザクション / WebSocket リスナーはルールをバイパスする（拡大は plan.md の残課題）
