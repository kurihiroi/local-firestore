...
- 段階的に機能を拡張していく
- **1 プロセス = 1 SQLite ファイル**: リアルタイム通知（リスナー購読）とトランザクションの
  OCC 状態はサーバープロセスのメモリ上に保持されるため、同一 SQLite ファイルに対して
  複数のサーバープロセスを起動することはできない。起動時に `<DB_PATH>.lock` ファイルで
  多重起動を検出し、既に生存プロセスがロックを保持している場合は起動をエラーにする
  （異常終了で残った stale ロックは自動回収）。水平スケールはスコープ外（ローカル用途）。
  複数インスタンスが必要な場合は `DB_PATH` を分けて起動する
- **at-rest 暗号化（オプション）**: `DB_ENCRYPTION_KEY` 環境変数を指定すると
  `better-sqlite3-multiple-ciphers` で SQLite ファイルを暗号化して開く
  （マルチデータベースの各ファイルにも同じ鍵を適用）。`:memory:` では無効。
  鍵の誤り・暗号化有無の不一致は起動時に検出してエラーにする。
  既存の平文 DB を暗号化する場合は、鍵なしで起動したサーバーから `GET /export` で
  エクスポートし、`DB_ENCRYPTION_KEY` 付きで新規起動したサーバーへ `POST /import` する
